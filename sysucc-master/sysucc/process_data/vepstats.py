    ...
    with open(maf) as fp:
        ...
        for row in reader:
            ...
            if row['Hugo_Symbol'] not in ['.', 'Unknown']:
                ...
                varclass_tmp['aachange'] = row['HGVSp_Short']
                ...
                elif tmp == 'Splice_Site':  # Splice_Region
...
def update_VariationClassification_rate():
    objs = VariationClassification.objects.all()
    for obj in objs:
        obj.total = obj.missense + obj.nonsense + obj.splice + obj.fsi + obj.fsd + obj.ifi + obj.ifd
        total = obj.total if obj.total != 0 else 1
        obj.missense_rate = obj.missense / total
        obj.nonsense_rate = obj.nonsense / total
        obj.splice_rate = obj.splice / total
        obj.fsi_rate = obj.fsi / total
        obj.fsd_rate = obj.fsd / total
        obj.ifi_rate = obj.ifi / total
        obj.ifd_rate = obj.ifd / total
        obj.save()


def update_SnvClass_rate():
    objs = SnvClass.objects.all()
    for obj in objs:
        obj.total = obj.t2g + obj.t2a + obj.t2c + obj.c2t + obj.c2g + obj.c2a
        total = obj.total if obj.total != 0 else 1
        obj.t2g_rate = obj.t2g / total
        obj.t2a_rate = obj.t2a / total
        obj.t2c_rate = obj.t2c / total
        obj.c2t_rate = obj.c2t / total
        obj.c2g_rate = obj.c2g / total
        obj.c2a_rate = obj.c2a / total
        obj.save()


def create_or_update_VariationClassificationTotalBar():
    '''需要先清理数据表格delete from
    '''
    total, missense, nonsense, splice, fsi, fsd, ifi, ifd = 0, 0, 0, 0, 0, 0, 0, 0
    objs = VariationClassification.objects.all()
    for obj in objs:
        total += obj.total
        ...
        ifi += obj.ifi
        ifd += obj.ifd
    total_ = total if total != 0 else 1
    missense_rate, nonsense_rate, splice_rate, fsi_rate, fsd_rate, ifi_rate, ifd_rate = missense / total_, nonsense / total_, splice / total_, fsi / total_, fsd / total_, ifi / total_, ifd / total_
    VariationClassificationTotalBar.objects.create(
        total=total, missense=missense, nonsense=nonsense,
        splice=splice, fsi=fsi, fsd=fsd, ifi=ifi, ifd=ifd,
        missense_rate=missense_rate, nonsense_rate=nonsense_rate,
        splice_rate=splice_rate, fsi_rate=fsi_rate,
        fsd_rate=fsd_rate, ifi_rate=ifi_rate,
        ifd_rate=ifd_rate
    )


def create_or_update_VariantTypeTotalBar():
    snp, ins, dels = 0, 0, 0
    objs = VariantType.objects.all()
    for obj in objs:
        snp += obj.snp
        ins += obj.ins
        dels += obj.dels
    VariantTypeTotalBar.objects.create(
        snp=snp, ins=ins, dels=dels
    )


def create_or_update_SnvClassTotalBar():
    total, t2g, t2a, t2c, c2t, c2g, c2a = 0, 0, 0, 0, 0, 0, 0
    objs = SnvClass.objects.all()
    for obj in objs:
        total += obj.total
        t2g += obj.t2g
        ...
        c2a += obj.c2a
    total_ = total if total != 0 else 1
    t2g_rate, t2a_rate, t2c_rate, c2t_rate, c2g_rate, c2a_rate = t2g / total_, t2a / total_, t2c / total_, c2t / total_, c2g / total_, c2a / total_
    SnvClassTotalBar.objects.create(
        total=total, t2g=t2g, t2a=t2a,
        t2c=t2c, c2t=c2t, c2g=c2g, c2a=c2a,
        t2g_rate=t2g_rate, t2a_rate=t2a_rate,
        t2c_rate=t2c_rate, c2t_rate=c2t_rate,
        c2g_rate=c2g_rate, c2a_rate=c2a_rate
    )
...
def update_SortedGeneVariationClassification_rate():
    objs = SortedGeneVariationClassification.objects.all()
    for obj in objs:
        obj.tsample_num_rate = obj.tsample_num / 1015
        obj.total = obj.missense + obj.nonsense + obj.splice + obj.fsi + obj.fsd + obj.ifi + obj.ifd
        total = obj.total if obj.total != 0 else 1
        obj.missense_rate = obj.missense / total
        obj.nonsense_rate = obj.nonsense / total
...
def update_or_create_oncoplot(all_gene_list, sample_list, ssv):
    '''更新需要先执行delete from cccga_oncoplot清空表格
    '''
